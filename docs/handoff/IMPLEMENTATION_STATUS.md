...
| Documents centre (holder-visible subset) | **PARTIALLY IMPLEMENTED** — visibility and retrieval work, but **no real file can be attached** (K-33) | e2e + both UIs | `set-document-visibility.ts`, `get-my-asset-documents.ts` | `assets-api.e2e`, `portfolio-api.e2e` | No per-holder access log, no watermarking, no versioning |
