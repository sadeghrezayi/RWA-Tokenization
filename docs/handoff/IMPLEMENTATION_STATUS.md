...
| Issuer portal (issuer-facing) | **PARTIALLY IMPLEMENTED** | web tests + layout contract + `issuers-api.e2e` | `apps/web/app/[locale]/issuer/`, `http/issuers.controller.ts` | `issuer-landing.test.tsx`, `use-browser-session.test.ts`, `layout.spec.ts`, `issuers-api.e2e` | Shipped: the portal shell, the landing page, "which organisations are mine" (3.3d/3.3e), and the read of **the assets an organisation brought** (3.3f backend, `GET /issuers/:id/assets`, membership-authorised + mutation-checked). **Not built:** any issuer-facing SCREEN for those assets, and the 13-step submission wizard — that scope is the owner's call |
