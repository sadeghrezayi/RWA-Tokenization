...
|---|---|---|---|---|---|---|
| K-1 | **Medium** (was High) | Issuer **team** management has no browser flow — adding, listing and removing an issuer's people is HTTP-only. The **review queue is now built** (3.2f, `/[locale]/admin/issuers`) | The UI is being delivered in slices; the review screen landed first | `apps/web/components/admin/issuers-panel.tsx` (exists), team panel (missing) | curl for team endpoints | Build the team panel, then the issuer-facing portal |
