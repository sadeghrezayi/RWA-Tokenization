...
|---|---|---|---|---|---|---|
| K-1 | **High** | Phase 3.2 is invisible: no browser flow exists for issuer applications or teams | The slice shipped API + wiring only; the UI slice was next when work paused | `apps/web` (missing), `http/issuers.controller.ts` (exists) | curl | Build the ops review screen (backlog P0-1) |
