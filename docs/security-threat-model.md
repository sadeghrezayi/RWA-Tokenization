...
|---|---|---|---|
| T1 | Administrator compromise | Med — **real staff Users with distinct least-privilege roles (1.4c)**; officer MFA (1.3d) + four-eyes on high-value credit (1.4b) | ✅ User + Membership + distinct roles, deny-by-default per-permission (1.4a/c); ✅ officer TOTP MFA opt-in (1.3d) — make mandatory for privileged roles (OD-23); ✅ maker-checker four-eyes (1.4b); per-action audit (have), session hardening (1.3), privileged-role grant approval + extend maker-checker to more actions (1.4d) |
...
| T3 | Insider abuse (staff) | High — **four-eyes now enforced on high-value ledger credit (1.4b)**; more sensitive actions still single-role | ✅ maker-checker engine + four-eyes on above-threshold ledger credit — self-approval rejected, distinct checker required, approvals queue (1.4b); extend to the full sensitive set (forced transfer, payout, tokenize, …) as roles split (1.4c/d); task/queue visibility (have), immutable audit incl. read-access to PII (4), role separation, alerts on unusual ops (8) |
...
| T8 | Double payment / duplicate credit | Medium — idempotent markPaid exists; **above-threshold credit now behind maker-checker (1.4b)** | ✅ threshold four-eyes on ledger credit (1.4b); Idempotency keys (1.6), Payment natural keys + matching (6), journal balance invariant (6.1), reconciliation jobs (6.4). Note: approve-then-execute is not yet transactionally atomic — outbox/idempotent execution is a 1.6 item |
...
| T16 | Privilege escalation | Low-Med — **deny-by-default per-permission guard (1.4a) + real User/Membership multi-role staff with least privilege (1.4c)** | ✅ granular `@RequirePermission` over all endpoints + role→permission map (1.4a); ✅ User + Membership, distinct least-privilege roles, token carries roles, unknown roles ignored (1.4c); per-role authz matrix tests + privileged-grant approval (1.4d); maker-checker on sensitive set (1.4b) |
